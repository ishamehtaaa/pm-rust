...
impl RiskManager {
    pub fn new(max_exposure: Decimal, max_exposure_per_market: Decimal) -> Self {
        Self {
            max_exposure,
    ...
    pub fn budget_limit_per_market(&self, usdc_balance: Decimal) -> Decimal {
        let cap = if usdc_balance < self.max_exposure_per_market {
            usdc_balance
        } else {
            self.max_exposure_per_market
        };
        cap.min(self.max_exposure)
    }
}
